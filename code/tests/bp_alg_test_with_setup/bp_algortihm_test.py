...
for n in logspace:
    ...
    time_opt_temp = 0
    time_non_temp = 0
    time_opt_setup_temp = 0
    time_non_setup_temp = 0
    time_opt_tot_temp = 0
    time_non_tot_temp = 0
    time_opt_tot_start = 0
    time_non_tot_start = 0
    it_opt_temp = 0
    it_non_temp = 0
    for i in range(q):
        ...
        time_opt_tot_start = t.time()
        res_optimal = l1.l1_admm(A,b,rho,maxit,er =er,es = es,quiet = True)
        time_opt_tot_temp += (t.time() - time_opt_tot_start)
        time_non_tot_start = t.time()
        res_nonoptimal = l1.l1_admm_nonoptimized(A,b,rho,maxit,er =er,es = es,quiet = True)
        time_non_tot_temp += (t.time() - time_non_tot_start)
        time_opt_temp += res_optimal[5]
        time_non_temp += res_nonoptimal[5]
        time_opt_setup_temp += res_optimal[7]
        time_non_setup_temp += res_nonoptimal[7]
        it_opt_temp += res_optimal[6]
        it_non_temp += res_nonoptimal[6]
			
    time_opt_tot.append(time_opt_tot_temp/q)	
    time_non_tot.append(time_non_tot_temp/q)	
    time_opt.append(time_opt_temp/q)
    time_opt_setup.append(time_opt_setup_temp/q)
    time_non_setup.append(time_non_setup_temp/q)
    it_opt.append(it_opt_temp/q)
    time_non.append(time_non_temp/q)
    it_non.append(it_non_temp/q)
    count = len(time_opt)
    data = np.array([logspace[:count],time_non,time_opt,it_non,it_opt,time_non_setup,time_opt_setup,time_opt_tot,time_non_tot])
