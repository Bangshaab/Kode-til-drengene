...
for file1,file2 in zip(glob.glob('data/X*')[::2],glob.glob('data/X*')[1::2]):
    ...
    print(file1)
    np.save('zetas_lag2/zeta'+file1[6:8]+'.npy',zeta)
...
plt.plot(zetas[0],label = 'bits = 5')
plt.plot(zetas[1],label = 'bits = 6')
plt.plot(zetas[2],label = 'bits = 7')
plt.plot(zetas[3],label = 'bits = 8')
plt.plot(zetas[-1],label = 'bits = 16')
plt.xlabel('Iteration [k]')
plt.legend()
plt.ylabel('Estimated correlation gain ($\\zeta^k$) [$\\cdot$]')
...
i = 0
plt.plot(mean_hats[i][1:], label= 'E[$\\hat{X}^k$]')
plt.plot(mean_oris[i], label= 'E[$X^k$]')

plt.plot(sigma_hats[i][1:], label= 'Var($\\hat{X}^k$)')
plt.plot(sigma_oris[i], label= 'Var($X^k$)')
plt.xlabel('Iteration [$\\cdot$]')
plt.legend()
plt.grid(alpha = .5)
