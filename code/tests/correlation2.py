...
for i in range(its):
    ...
    #print(x_start)
    aaa = np.zeros(100)
    res = lasso.lassoQuantPre(A,b,rho,lamda,100,aaa,xPre = x_start,uEst = u_start,zEst = z_start,err = -1,rounding = True,roundSpecs=[-1.5,1.5,int(bits)]) # Lasso
    X = np.array(res[0]) # History of the x vector
    Xhat = np.array(res[1])
    ...
    '''
np.save('data/X'+str(bits)+'_ori.npy',np.array(data_ori))
np.save('data/X'+str(bits)+'_hat.npy',np.array(data_hat))
exit()
