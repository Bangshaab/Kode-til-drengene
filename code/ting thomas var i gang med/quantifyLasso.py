...
for l in range(16):
    res = lasso(A,b,rho,lmbda,100,rounding=True,roundSpecs=[lowerBound,upperBound,l+1])
    print(res[6])
    if l > 0:
        ...
        itt = np.array(res[6])
print(itt[13],itt[14])
print(histX[np.cumsum(itt):np.cumsum(itt[0]),:].shape)
plt.plot(histX[itt[13]:itt[14],:])
plt.show()
