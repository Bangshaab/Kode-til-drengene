...
def lassoQuantPre(A,b,rho,lmbda,maxit,alpha,x = True,z = True,u = True,err=10**(-9),rounding = False,roundSpecs = [1,1,1]):  
  ...
  while (r_norm + s_norm > err) and i < maxit:
    ...
    zEstOld = zEst.copy()
    zEst = rounderino(limits,a,soft(uEst+xPre,lmbda/rho))
    uEst = rounderino(limits,a,uEst + xPre - zEst)
  ...
  return [histXEst, histZEst, histUEst, historyr, historys, i,zEst]
...
for i in range(reps):
    ...
    solved.append(np.allclose(xw,np.around(res[6]).copy().astype(int)))

    tempJ = np.asarray(res[3]) + np.asarray(res[4])
    R[i,:] = tempJ[1:].copy()

    if i%10==0:
        print('Bitdepth: %d, '%bitDepth,i,'of',reps)

np.save('R%d_solved=%d of %d.npy'%(bitDepth,sum(solved),reps),res[6])